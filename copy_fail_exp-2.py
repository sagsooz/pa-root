#!/usr/bin/env python3
import os as g, zlib, socket as s, ctypes, ctypes.util
...
def splice(fd_in, fd_out, length, offset_src=None):
    ...
    off_in_ptr = ctypes.byref(off_in) if off_in is not None else None
    ret = _libc.splice(
        ctypes.c_int(fd_in), off_in_ptr,
        ctypes.c_int(fd_out), None,
